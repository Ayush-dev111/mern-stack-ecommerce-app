...
const {cart} = useCartStore();
  return (
    ...
    transition-all duration-300 border-b border-blue-700'>

      <div className='container mx-auto px-4 py-3'>  
        <div className="flex flex-wrap items-center justify-between">
          <Link to='/' className='text-2xl font-bold text-blue-500 items-center space-x-2 flex'>
          E-commerce
          </Link>

          <nav className="flex flex-wrap items-center gap-4">
            <Link to='/' className='text-gray-300 hover:text-indigo-400 transition duration-300 ease-in-out text-lg'>
              Home
            </Link>
            ...
            {isAdmin && (
						...
						{user ? (
							<button
								className='bg-gray-700 hover:bg-gray-600 text-white py-1a px-4 
